...
@onready var pip_1: Panel = get_node_or_null("BossHUDContainer/PanelContainer/VBox/PipContainer/Pip1")
@onready var pip_2: Panel = get_node_or_null("BossHUDContainer/PanelContainer/VBox/PipContainer/Pip2")
@onready var pip_3: Panel = get_node_or_null("BossHUDContainer/PanelContainer/VBox/PipContainer/Pip3")
...
# White lagging bar catch-up duration (0.4s)
const CATCHUP_DURATION: float = 0.40
...
func _process(delta: float) -> void:
	...
	# Primary health bar response
	current_displayed_health = move_toward(current_displayed_health, target_health, delta * (max_health * 1.5))
	if health_bar:
		health_bar.value = current_displayed_health
	
	# White lagging damage bar (catches up smoothly over 0.4s)
	if ghost_health > target_health:
		var catchup_rate = max_health / CATCHUP_DURATION
		ghost_health = move_toward(ghost_health, target_health, delta * catchup_rate)
	else:
...
func activate_boss(boss_name: String = "GOLIATH // BIO-WEAPON EXPERIMENT 00", hp: float = 2500.0, m_hp: float = 2500.0) -> void:
	is_active = true
	...
	if ghost_bar:
		...
		ghost_bar.value = ghost_health
	if boss_name_label:
		boss_name_label.text = boss_name if boss_name != "" else "GOLIATH // BIO-WEAPON EXPERIMENT 00"
	if phase_label:
		phase_label.text = "PHASE 1: HEAVY CLEAVE & GROUND SMASH"
	...
	if phase_label:
		match phase:
			1:
				...
				phase_label.modulate = Color(0.95, 0.75, 0.2)
			2:
				phase_label.text = "PHASE 2: BULL RUSH & BOULDER HURL"
				phase_label.modulate = Color(1.0, 0.4, 0.15)
			3:
				phase_label.text = "PHASE 3: CORE MELTDOWN & HORDE RALLY"
				phase_label.modulate = Color(1.0, 0.1, 0.25)

func _update_pips(phase: int) -> void:
	var active_color = Color(1.0, 0.2, 0.25, 1.0)
	var inactive_color = Color(0.25, 0.28, 0.32, 0.8)
	var cleared_color = Color(0.18, 0.85, 0.35, 1.0)
	
	if pip_1:
		pip_1.modulate = cleared_color if phase > 1 else (active_color if phase == 1 else inactive_color)
	if pip_2:
		pip_2.modulate = cleared_color if phase > 2 else (active_color if phase == 2 else inactive_color)
	if pip_3:
		pip_3.modulate = active_color if phase == 3 else inactive_color
